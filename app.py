# DK Golf — PGA Optimizer (Single Entry • Balanced Cash)
# Run with:
...
avg_col = find_col(dk, ["avgpointspergame", "avg_points_per_game"])

if name_col is None or salary_col is None:
    st.error("Could not detect DK name or salary column.")
    st.stop()
...
if dg_file:
    ...
    if dg_name:
        ...
        if all(c in dk.columns for c in [col_app, col_ott, col_t2g, col_total]):
            ...
            cs = dk["cut_safety_z"]
            dk["cut_safety"] = (
                100 * (cs - cs.min()) / (cs.max() - cs.min() + 1e-9)
            )
...
dk["se_proj"] = dk["base_proj"] * (
    1 + 0.06 *
    (0.75 * dk["proj_z"] + 0.25 * dk["cut_z2"]).clip(-2, 2) / 2
)

# Remove bad rows early
dk = dk.replace([np.inf, -np.inf], np.nan)
dk = dk.dropna(subset=["se_proj", salary_col])
...
if st.button("Generate Lineup"):
    ...
    x = [
        LpVariable(f"x{i}", 0, 1, LpBinary)
        for i in range(len(pool))
    ]

    prob += lpSum(pool.loc[i, "se_proj"] * x[i] for i in range(len(pool)))
    prob += lpSum(x) == ROSTER_SIZE
    prob += lpSum(pool.loc[i, salary_col] * x[i]
                  for i in range(len(pool))) <= SALARY_CAP
    ...
    st.metric("Salary Used",
              f"${int(lineup[salary_col].sum()):,}")
    st.metric("Projected Points",
              f"{lineup['se_proj'].sum():.2f}")

    st.dataframe(
        lineup[[name_col, salary_col,
                "se_proj", "cut_safety"]],
        use_container_width=True
    )
